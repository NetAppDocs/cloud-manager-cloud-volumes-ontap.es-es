...
Estos permisos se incluyen en https://docs.netapp.com/us-en/cloud-manager-setup-admin/reference-permissions-aws.html["Las políticas proporcionadas por NetApp"^]
...
+
Cuando vaya a la página Advanced Allocation, puede identificar si la función Elastic Volumes está habilitada en un agregado. En el ejemplo siguiente, aggr2 tiene volúmenes Elastic habilitados, mientras que aggr1 no lo tiene.

+
image:screenshot-elastic-volumes-enabled.png["Captura de pantalla que muestra dos agregados en los que uno tiene un campo con el texto volúmenes Elastic habilitados."]
